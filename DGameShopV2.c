    ...
    while (percobaan < 3 && !berhasil) {
        ...
        while (fread(&data_user, sizeof(data_user), 1, fuser) == 1) {
            if (strcmp(username, data_user.username) == 0 && strcmp(pw, data_user.password) == 0) {
                ...
                berhasil = 1;
                current_user = data_user; // <-- perbaikan di sini
                fclose(fuser);
                system("pause");
                MenuUser();
                return;
...
if(friwayat) {
    ...
    // Format: NamaGame,Username,Harga,PotonganDev,PotonganAdmin
...
void Wallet(){
    ...
    // Update saldo current_user agar selalu sinkron
...
void tambahWallet() {
    ...
    for (int i = 0; i < jumlah; i++) {
        if (strcmp(users[i].username, current_user.username) == 0) {
            ...
            current_user.saldo = users[i].saldo; // update saldo user yang sedang login
...
void ProfileUser(){
     int pilihan;
    // struct User user; // Hapus baris ini
